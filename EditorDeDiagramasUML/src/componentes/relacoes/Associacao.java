...
public class Associacao extends RelacaoUML{
    ...
    public Point getLocalizacaoDoPontoDeExtensao() {
        Rectangle boundsUltimaLinha = getLinhasDaRelacao().get(getLinhasDaRelacao().size() - 1).getBounds();
        Point localizacaoPontoDeExtensao = new Point();

        switch (getOrientacaoDaRelacao()) {
            case SUL -> {
                localizacaoPontoDeExtensao.x = boundsUltimaLinha.x + ((boundsUltimaLinha.width - TAMANHO_PONTO_DE_EXTENSAO)/2);
                localizacaoPontoDeExtensao.y = boundsUltimaLinha.y + boundsUltimaLinha.height - TAMANHO_PONTO_DE_EXTENSAO/2;
            }
            case NORTE -> {
                localizacaoPontoDeExtensao.x = boundsUltimaLinha.x + ((boundsUltimaLinha.width - TAMANHO_PONTO_DE_EXTENSAO)/2);
                localizacaoPontoDeExtensao.y = boundsUltimaLinha.y - TAMANHO_PONTO_DE_EXTENSAO/2;
            }
            case OESTE -> {
                localizacaoPontoDeExtensao.x = boundsUltimaLinha.x - TAMANHO_PONTO_DE_EXTENSAO/2;
                localizacaoPontoDeExtensao.y = boundsUltimaLinha.y + ((boundsUltimaLinha.height - TAMANHO_PONTO_DE_EXTENSAO)/2);
            }
            case LESTE -> {
                localizacaoPontoDeExtensao.x = boundsUltimaLinha.x + boundsUltimaLinha.width - TAMANHO_PONTO_DE_EXTENSAO/2;
    ...
    protected void initFrameGerenciarRelacao() {
        ...
        painelAplicar.setBackground(gerenciadorDeRecursos.getColor("white"));
        painelAplicar.setBorder(BorderFactory.createMatteBorder(
            1, 1, 1, 1, gerenciadorDeRecursos.getColor("raisin_black")
        ));

        JLabel labelAplicar = new JLabel(gerenciadorDeRecursos.getString("aplicar"));
        labelAplicar.setFont(gerenciadorDeRecursos.getRobotoMedium(14));
        ...
        painelAplicar.addMouseListener(adapterCorDosBotoes);
